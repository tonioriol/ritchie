# Deploy Acestreamio (Stremio addon) to neumann
...
If you deploy [`apps/external-dns.yaml`](apps/external-dns.yaml:1), the DNS record for the Ingress host can be created/maintained automatically (no `doctl` step required).
