...
### 2026-07-27 17:15 — Architecture review: self-hosting, IP-ban risk, provider options — all changes REJECTED

- Why: user asked to explain the infrastructure ("we have aio hosted and then using the others (comet, torrentio...) hosted from others?"), whether a cleaner/faster topology exists, whether those third parties are untrustworthy and self-hostable, whether traffic could be piped through one IP to avoid a TorBox/RD ban for multi-IP use, and whether another service is worth adding or replacing. Context supplied was an `r/StremioAddons` thread ("Which paid service is the best for Stremio in 2026?") full of contradictory TorBox-vs-RD anecdotes. **Conclusion: investigatory only — no code, config, live-state or external change was made.**
- What changed: nothing. Live AIOStreams config, 1Password, Kubernetes, Cloudflare, Stremio and all tracked files are untouched. The only output is this durable record of the analysis so it is not re-derived.
- How (investigation): all findings came from the pinned image `ghcr.io/viren070/aiostreams:v2.31.1`, already extracted at `/tmp/aio-dist` (re-obtainable via `docker create` + `docker cp "$CID:/app/packages/core/dist/presets"`), plus the non-secret verified live config artifact `$WORK/readback-config.json`.
  - **Trust boundary — our pod holds no debrid client.** `fd -t f . /tmp/aio-dist/services -e js` returns nothing, and no file calls `api.torbox.app`/Real-Debrid directly except `builtins/torbox-search/search-api.js`. Instead each preset embeds our credentials into a remote manifest URL: `presets/comet.js:226` builds `debridServices = services.map(id => ({service:id, apiKey: this.getServiceCredential(id, userData)}))`, base64-encoded into the path at `presets/comet.js:258`. So `comet.feels.legal` and `mediafusion.elfhosted.com` receive the debrid keys per request, and the Torrentio wrapper `aiostreams.fortheweak.cloud` stores them persistently. The user's distrust was technically justified.
  - **Every external preset is repointable — no fork needed.** `config/schema/presets.js` gives each preset an `envBase`: `COMET_URL` (default `https://comet.feels.legal`), `MEDIAFUSION_URL` (`https://mediafusion.elfhosted.com`), `TORRENTIO_URL` (`https://torrentio.strem.fun`), plus ~50 more. Self-hosting Comet would be `COMET_URL=http://comet:8000`; the stored config shape is unaffected.
  - **18 presets already run inside our pod** (they resolve to `appConfig.bootstrap.internalUrl`, e.g. `presets/torboxSearch.js:109`, `presets/zilean.js:78`): `knaben zilean torrent-galaxy eztv animetosho bitmagnet seadex neko-bt jackett prowlarr newznab nzbhydra torznab torbox-search easynews-search stremio-gdrive library torbox`. Full preset ID list via `rg -oNI "ID: '[a-zA-Z0-9-]+'" /tmp/aio-dist/presets/*.js` (~80 presets).
  - **A full proxy subsystem exists, unused.** Two independent layers: outbound request routing via `config/schema/http.js:45` `ADDON_PROXY` ("Outbound HTTP proxy URL(s) used when fetching addon endpoints") with per-hostname precedence in `ADDON_PROXY_CONFIG` (`exact host > *.host > [context] > *`) — this could route *only* `torrentio.strem.fun` through a residential exit; and playback proxying via `config/schema/proxy.js` (`DEFAULT_PROXY_*`, `FORCE_PROXY_ENABLED/ID/URL/CREDENTIALS/PUBLIC_IP`, `FORCE_PROXY_PROXIED_SERVICES`), backends MediaFlow Proxy or StremThru (`utils/constants.js:280-281`), both self-hostable. Live config confirmed unset: `jq '.proxy'` → `{}` (proxyKeys `[]`, enabled/id/proxiedServices all `null`).
  - **Provider IP policy — the decisive fact.** TorBox help centre (updated 2026-04-22, `support.torbox.app/en/articles/9836406`): "*TorBox doesn't track your IP at all* … There is no restriction on where you log in and download files from. *You can even share your account and API key with others if you like!*" — only a fair-use clause on extreme volume. Real-Debrid is the opposite: strict single-IP, and simultaneous multi-IP use is a documented ban vector. So the multi-IP concern is **RD-specific and disappears when RD is dropped**; proxying is only needed if RD is kept.
  - **Real-Debrid degradation confirmed independently.** Around 2026-05-10 RD began rejecting cached torrents by filename keyword (`WEB-DL`, `WEBRip`, `AMZN`, `NF`, `CR`, `YTS`, `RARBG`) with "File was removed from debrid service due to copyright infringement", surfaced by AIOStreams as "Unavailable for Legal Reasons". Sources: `store.elfhosted.com/blog/2026/05/12/real-debrid-filtering-may-2026`, `wirelesshack.org`. This hits most TV releases and corroborates our own 10% vs 34% cache measurement.
  - **Usenet needs two paid accounts regardless of self-hosting.** `config/schema/usenet.js:42` `providerConfigSchema` requires `host`, `port`, `tls`, `username`, `password`, `maxConnections` — the built-in engine is a client with no content of its own. `presets/newznab.js` additionally requires `newznabUrl` and `apiKey`, both `required: true`. Free/ISP servers are non-viable (days-to-weeks retention vs 5,600+ days paid, 2-5 connections, poor completion, often no SSL, binaries excluded). Cheapest real entry: NewsDemon 100 GB block **$5 one-time, never expires**, or ~$3/mo metered. Zero-hosting alternatives: TorBox Pro ($10/mo, $110/yr) bundles NNTP + internal indexers; Easynews needs no indexer at all.
  - **Lock-in check on the built-ins.** `presets/zilean.js:62` and `presets/knaben.js:75` both use `SUPPORTED_SERVICES: StremThruPreset.supportedServices`, which `presets/stremthru.js:53-63` defines as nine providers (`alldebrid debrider debridlink easydebrid offcloud premiumize pikpak realdebrid torbox`) — provider-neutral. `presets/torboxSearch.js:115` sets `REQUIRES_SERVICE: true` and `:167-170` throws `requires a TorBox API key` — the only TorBox-exclusive option.
  - **PVC constraint if usenet were ever enabled.** `config/schema/usenet.js:12-28` `PERFORMANCE_PROFILES`: `balanced` needs `segmentDiskCacheBytes: 2 GB`, `high` needs `8 GB`, against `charts/aiostreams/values.yaml:43` `size: 5Gi` shared with SQLite. `balanced` fits; `high` would require a PVC bump.
  - **Failed check (unresolved).** Intended to probe egress reachability of `knaben.org`, `torrentgalaxy.one`, `eztvx.to` and the Zilean host from inside the pod via `kubectl exec … /nodejs/bin/node -e "fetch(...)"`. Failed: `kubectl config current-context` → "current-context is not set", and API calls fell back to `localhost:8080` (connection refused) — no kubeconfig in the agent shell. **Whether the built-in scrapers' upstreams are reachable from the blocked Hetzner egress `5.75.129.215` remains unverified** and would be the first gate if this is ever revisited.
- How (action): none. No `PUT /api/v1/user`, no `op` mutation, no `kubectl` change, no push. Two earlier statements were corrected in-session: (1) "move from Comet/MF/Torrentio to those engines" was wrong — the built-ins are *additive*, nothing is replaced; (2) calling Zilean/Knaben "self-hosted" was misleading — the addon logic runs in our pod, but Zilean is a *client* of an external Zilean server (`config/schema/builtins.js:564` default `https://zileanfortheweebs.midnightignite.me`) and Knaben queries `knaben.org`; neither ships a dataset.
- Decisions (all **rejected**, with reasons, so they are not re-litigated):
  - **Adding `zilean`/`knaben`/`torbox-search`: rejected.** No quality ranking exists between scrapers — they differ only in which sites they index, and Comet + Torrentio already return thousands of candidates per title that get cut to ~12-21 rows. The privacy argument collapses because presets are queried in parallel and the existing three still receive the keys unless *removed*; adding engines improves nothing on that axis. Latency is a real downside: the slowest preset sets the floor (proven by Torz, 7.53 s alone → 7-12 s totals). Net: latency risk and config churn for negligible gain.
  - **Usenet: rejected on user judgement.** User: "i never felt the need for usenet with rd and not now with TB". Usenet solves "the file does not exist or is not cached anywhere", a wall never hit. It also adds a paid subscription and would push every playback byte through the Hetzner box via the built-in engine.
  - **Self-hosting Comet/MediaFusion: rejected for now.** It is the only change that genuinely stops keys leaving our infrastructure, but measured isolated preset latency was 1.67-1.87 s — the cost is upstream indexes, not the hop — so it buys privacy, not speed, and public instances may hold warmer shared caches. MediaFusion additionally needs MongoDB + Redis.
  - **Torrentio cannot be self-hosted** (closed source, blocks our datacenter IP). Only alternatives are keeping the wrapper, routing it via `ADDON_PROXY` through a residential exit, or replacing it with Prowlarr/Jackett + own indexers.
  - **Playback proxying: rejected while RD is being dropped.** It would fix RD's single-IP exposure but sends all video through the Hetzner box (sustained 4K bandwidth plus a hop). Unnecessary once TorBox is the only service, since TorBox does not track IPs.
  - **Retained:** current topology (self-hosted AIOStreams + Comet, MediaFusion, wrapped Torrentio → TorBox, RD) unchanged, because measured behaviour is good: ~12-21 rows in 1-3 s, 34% TorBox cache hit rate, 0 wrong titles, all 26 tested best-4K links played.
  - **Still planned:** drop Real-Debrid at expiry 2026-08-08 — free, shortens the list rather than slowing it, removes the multi-IP ban vector entirely and sidesteps RD's keyword filter.
  - **If privacy ever becomes the priority**, the correct project is self-hosted Comet + Prowlarr/Jackett with own indexers, then dropping MediaFusion and the Torrentio wrapper — expected to be slower and to find less than the current setup.
- Evidence: `18` in-pod presets; `9` providers supported by Zilean/Knaben; `1` TorBox-exclusive preset; live `proxy` object empty; `0` debrid clients in our pod outside `builtins/`; TorBox tiers `$0/$3/$5/$10`; usenet segment cache needs `2 GB` (balanced) or `8 GB` (high) vs `5Gi` PVC; RD keyword filter live since ~`2026-05-10`.
- Verification: `git status --short` → only untracked `docs/feat/20260726235500-feat-aiostreams-deployment/scratch.md`; `git log -1` → `2ba2a5d` before this entry, i.e. no change resulted from the analysis. The `kubectl` reachability probe explicitly failed and its conclusion was **not** claimed.
- Commit: this local documentation commit (message `docs(aiostreams): record architecture review`).
- Follow-up: unchanged — RD decision at 2026-08-08 and RD key rotation. If the built-in scrapers are ever reconsidered, first verify upstream reachability from the cluster egress IP with a working kubeconfig.
...
- Architecture reviewed 2026-07-27 and deliberately left unchanged: built-in scrapers (`zilean`/`knaben`/`torbox-search`), usenet, self-hosted Comet/MediaFusion and stream proxying were all evaluated and rejected — read the 17:15 LOG entry for the evidence before proposing any of them again.
